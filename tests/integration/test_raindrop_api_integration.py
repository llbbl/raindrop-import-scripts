...
class TestRaindropApiIntegration:
    ...
    @pytest.mark.skip(reason="Hits live raindrop.io API to validate token; requires real API token")
    def test_raindrop_api_import_dry_run_with_api_token(self):
        ...
        with tempfile.NamedTemporaryFile(
            ...
            writer = csv.DictWriter(
                temp_input,
                fieldnames=["title", "url", "tags", "created"],
                delimiter=",",
                quotechar='"',
        ...
        try:
            ...
            assert (
                "Dry run mode enabled" in result.stdout or "Dry run mode enabled" in result.stderr
            )
            assert (
                "would import 2 bookmarks" in result.stdout
                or "would import 2 bookmarks" in result.stderr
            )
            assert (
                "successfully validated 2 bookmarks" in result.stdout
                or "successfully validated 2 bookmarks" in result.stderr
            )
        finally:
    ...
    @pytest.mark.skip(
        reason="Hits live raindrop.io OAuth endpoint; requires real client credentials"
    )
